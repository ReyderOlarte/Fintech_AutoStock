...
public class XYSmoothLineAndShapeRenderer extends XYLineAndShapeRenderer {
	...
	public static Point2D.Double[] getBezierCurve(Point2D.Double point0, Point2D.Double point1, Point2D.Double point2, Point2D.Double point3, double smooth, int steps) {
		...
		Point2D.Double B = new Point2D.Double(3 * (control2.x - control1.x) - C.x, 3 * (control2.y - control1.y) - C.y);
		...
		for (int i = 1; i < steps; i++) {
			res[i] = new Point2D.Double(A.x * Math.pow(step, 3) + B.x * Math.pow(step, 2) + C.x * step + point1.x, A.y * Math.pow(step, 3) + B.y * Math.pow(step, 2) + C.y * step + point1.y);
